...
## 0.22.0 additions / 追加範囲

0.22.0では、既存の範囲にApplied Energistics 2（AE2）のツールチップ120項目を追加します。AE2の独立native確認は37ケース（実helper 27、caller-shaped fixture 10）で、英語・日本語・英語の同一Component再解決を確認しました。CPU数量とアイテム名、進行率と経過時間、Serial、Matter量、Requestable、Side結合、0秒・未満秒・時分秒を確認しています。ゲーム起動・画面目視確認は0件です。正数の時分秒リテラルはAE2側の実装に残り、Days/Hours/Minutesのregistry formatterは現行caller未確認です。Side名の結合に由来する空白・読点は既存資産側の外部依存として扱います。helper-0.4.0-devは別配布で、46既存ラベルを維持し新しい言語キーは追加しません。

Version 0.22.0 adds 120 Applied Energistics 2 tooltip entries. The independent AE2 native check covered 37 cases (27 actual helper cases and 10 explicitly marked caller-shaped fixtures), including locale cycling through the same Component, CPU quantity/item composition, progress/elapsed time, serial values, Matter quantity, requestable values, side composition and duration forms. Game boot and visual screen QA remain 0. Positive h/m/s literals remain source-side behavior, and registry-only day/hour/minute formatters have no confirmed current caller. The separate helper-0.4.0-dev keeps 46 existing captions and adds no language keys.



ATM11 Japanese project / **ueda-keisuke** provides this unofficial Japanese language collection for Minecraft 26.1.2 / NeoForge 26.1.2.106. Version 0.22.0 contains 4,089 entries across 55 namespaces: 4,068 translations, 19 preserved original metadata entries and two project notices. The release adds 120 Applied Energistics 2 tooltip entries. The AE2 review covers 37 native formatter cases; game visual QA remains 0. Existing NeoForge and Balm review evidence is retained. Native ordinary language-asset loading and formatter checks pass; FML early-loader pack application remains unverified, and visual game-screen checks remain 0. The original metadata comprises JEI `_comment`, Jade `__comment` / `jade.metadata`, QuarryPlus `_comment`, Nature's Compass `_comment`, and Enchantment Descriptions `_comment`, `__comment_jei` and 11 `__support_*` entries, plus Measurements `_comment`. Changes comprise Japanese names, descriptions, narration and search terms, with the metadata and project-notice exceptions below. Language modification date: **2026-09-20 (UTC)**.
This is the 0.22.0 release documentation. Game-screen visual QA remains 0, and FML early-loader pack application is unverified.
