...
## 0.24.0 additions / 追加範囲

0.24.0では、既存の範囲にApplied Energistics 2（AE2）の説明文・アイテム名など238言語項目を追加し、AE2は合計538項目です。既存0010–0014の300項目に、0003・0015・0016・0017（60+60+60+58）の新238項目を追加しています。4件すべて独立レビュー済みで、新238項目は、実際の言語読み込み・表示処理を使った33ケースと、アイテム／流体の返却によるロック解除処理2系列で検証済みです。表示用の受信データ、水の名称、蓄電量の文字列、TOPの引数には明示した検証用データを使っています。ゲーム全体の起動や画面描画の確認ではありません。ゲーム起動・画面目視確認は0件です。既存AE2証拠には59 formatter casesと別枠のFluix tooltip sequenceがあります。正数の時分秒リテラルとSide名の空白・読点はソース側の挙動として残ります。TrashControllerとTypeFilterの直接callerは未確認で、当該経路の完全な実画面表示を主張しません。helper-0.4.0-devは別配布で、46既存ラベルを維持し新しい言語キーを追加しません。

Version 0.24.0 adds 238 Applied Energistics 2 language entries from batches 0003, 0015, 0016 and 0017 (60+60+60+58), bringing the AE2 scope to 538. Batches 0010–0014 are the existing 300-entry scope. New batches 0003, 0015, 0016 and 0017 add 238 entries (60+60+60+58); all four final reviews are accepted. The new scope passes 33 offline native provider/formatter cases and two actual item/fluid return-lock transition sequences. Provider NBT data, the fluid display name, power strings and TOP arguments use explicit test fixtures; these checks do not establish a full MOD lifecycle or graphical frontend. Game boot and visual screen QA remain 0. Existing AE2 evidence covers 59 formatter cases plus a separate Fluix tooltip sequence. Positive h/m/s literals and Side-list punctuation remain source-side behavior; direct callers for TrashController and TypeFilter remain unconfirmed. The separate helper-0.4.0-dev keeps 46 existing captions and adds no language keys.



ATM11 Japanese project / **ueda-keisuke** provides this unofficial Japanese language collection for Minecraft 26.1.2 / NeoForge 26.1.2.106. Version 0.24.0 contains 4,507 entries across 55 namespaces: 4,486 translations, 19 preserved original metadata entries and two project notices. The release carries 238 new Applied Energistics 2 language entries, for 538 AE2 entries total. The existing 300-entry scope (0010–0014) and the new 238-entry scope (0003, 0015, 0016 and 0017) all have accepted final reviews. The new scope passes 33 offline native provider/formatter cases and two actual item/fluid return-lock transition sequences. Provider NBT data, the fluid display name, power strings and TOP arguments use explicit test fixtures; these checks do not establish a full MOD lifecycle or graphical frontend. Existing NeoForge and Balm review evidence is retained. Native ordinary language-asset loading and formatter checks pass; FML early-loader pack application remains unverified, and visual game-screen checks remain 0. The original metadata comprises JEI `_comment`, Jade `__comment` / `jade.metadata`, QuarryPlus `_comment`, Nature's Compass `_comment`, and Enchantment Descriptions `_comment`, `__comment_jei` and 11 `__support_*` entries, plus Measurements `_comment`. Changes comprise Japanese names, descriptions, narration and search terms, with the metadata and project-notice exceptions below. Language modification date: **2026-09-20 (UTC)**.
This is the 0.24.0 release documentation. Game-screen visual QA remains 0, and FML early-loader pack application is unverified.
...
Target: **26.1.10-beta**, 538 language entries across batches 0003 and 0010–0017. Batches 0010–0014 contain the 300 previously accepted AE2 entries; batches 0003, 0015, 0016 and 0017 add 238 entries. All four new final reviews are accepted. The new scope passes 33 offline native provider/formatter cases and two actual item/fluid return-lock transition sequences. Provider NBT data, the fluid display name, power strings and TOP arguments use explicit test fixtures; these checks do not establish a full MOD lifecycle or graphical frontend. The fixed English language entry SHA-256 is `0d9f11ad01d411f780a81fce45e73ef76bb7802abb2a628dc3feb8c37e33a880`; the existing Japanese entry SHA-256 is `405239e3613f55ac56e71ce62835d8fc33e9bb7d5c0c5c646f90840b86322aa8`. Both match the fixed source and installed JAR byte-for-byte. Existing Japanese provenance is retained; the official README identifies Crowdin localization contributors and the upstream contributor record remains the source of attribution.
