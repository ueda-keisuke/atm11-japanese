...
## 0.23.0 additions / 追加範囲

0.23.0では、既存の範囲にApplied Energistics 2（AE2）の説明文・アイテム名など180言語項目を追加し、AE2は合計300項目です。0012・0013・0014の各最終独立レビューは受理済みです。AE2の独立native確認は59 formatter casesと別枠のFluix tooltip sequenceで、10件の実AEKey数量formatter呼出し、8件の実ItemStack emptying-tooltip呼出しを含みます。ゲーム起動・画面目視確認は0件です。正数の時分秒リテラルとSide名の空白・読点はソース側の挙動として残ります。TrashControllerとTypeFilterの直接callerは未確認で、当該経路の完全な実画面表示を主張しません。helper-0.4.0-devは別配布で、46既存ラベルを維持し新しい言語キーを追加しません。

Version 0.23.0 adds 180 Applied Energistics 2 language entries, bringing the AE2 scope to 300. Batches 0012–0014 have independently accepted final reviews. The independent AE2 native evidence covers 59 formatter cases plus a separate Fluix tooltip sequence, including 10 actual AEKey amount formatter calls and 8 real ItemStack emptying-tooltip calls with explicit name payload fixtures. Game boot and visual screen QA remain 0. Positive h/m/s literals and Side-list punctuation remain source-side behavior; direct callers for TrashController and TypeFilter remain unconfirmed. The separate helper-0.4.0-dev keeps 46 existing captions and adds no language keys.



ATM11 Japanese project / **ueda-keisuke** provides this unofficial Japanese language collection for Minecraft 26.1.2 / NeoForge 26.1.2.106. Version 0.23.0 contains 4,269 entries across 55 namespaces: 4,248 translations, 19 preserved original metadata entries and two project notices. The release adds 180 Applied Energistics 2 language entries, for 300 AE2 entries total. All three added batches have independently accepted final reviews. Existing NeoForge and Balm review evidence is retained. Native ordinary language-asset loading and formatter checks pass; FML early-loader pack application remains unverified, and visual game-screen checks remain 0. The original metadata comprises JEI `_comment`, Jade `__comment` / `jade.metadata`, QuarryPlus `_comment`, Nature's Compass `_comment`, and Enchantment Descriptions `_comment`, `__comment_jei` and 11 `__support_*` entries, plus Measurements `_comment`. Changes comprise Japanese names, descriptions, narration and search terms, with the metadata and project-notice exceptions below. Language modification date: **2026-09-20 (UTC)**.
This is the 0.23.0 release documentation. Game-screen visual QA remains 0, and FML early-loader pack application is unverified.
