...
## 0.25.0 additions / 追加範囲

0.25.0では、既存の範囲にApplied Energistics 2（AE2）の説明文・アイテム名など181言語項目を追加し、AE2は合計719項目です。既存538項目に、0004・0005・0006・0007（60+60+60+1）の新181項目を追加しています。04v2・05v4・06v5・07 Of v3の最終レビューが完了しています。native-finalはformatter 37ケース・consumer 17系列でPASSし、GUI目視は0件です。公開packageゲートは別途です。呼び出し元の引数形状に合わせた明示的な検証用データを一部で使っています。これはゲーム全体の起動や画面描画の確認ではありません。ゲーム起動・画面目視確認は0件です。既存AE2証拠には59 formatter casesと別枠のFluix tooltip sequenceがあります。正数の時分秒リテラルとSide名の空白・読点はソース側の挙動として残ります。TrashControllerとTypeFilterの直接callerは未確認で、当該経路の完全な実画面表示を主張しません。helper-0.5.0-devは別配布で、46既存ラベルを維持し、Fluix材料の追加修復を含む44オフライン検証をPASSしています。Fluix材料欄の問題はhelper適用時に修復されます。

Version 0.25.0 adds 181 Applied Energistics 2 language entries from batches 0004, 0005, 0006 and 0007 (60+60+60+1), bringing the AE2 scope to 719. The previously accepted scope contains 538 entries; batches 0004, 0005, 0006 and 0007 are the new accepted scope. New batches 0004, 0005, 0006 and 0007 add 181 entries (60+60+60+1); all four final review records for batches 0004, 0005, 0006 and 0007 are present; the Of record covers its single assigned key. Current native checks cover 37 formatter cases and 17 consumer sequences with visual_qa=0; the public package gate remains separate. Some inputs use explicit fixtures shaped after the source callers; these checks do not establish a full MOD lifecycle or graphical frontend. Game boot and visual screen QA remain 0. Existing AE2 evidence covers 59 formatter cases plus a separate Fluix tooltip sequence. Positive h/m/s literals and Side-list punctuation remain source-side behavior; direct callers for TrashController and TypeFilter remain unconfirmed. The separate helper-0.5.0-dev keeps 46 existing captions and passes 44 offline verification cases including the Fluix-material repair; it is not bundled.



ATM11 Japanese project / **ueda-keisuke** provides this unofficial Japanese language collection for Minecraft 26.1.2 / NeoForge 26.1.2.106. Version 0.25.0 contains 4,688 entries across 55 namespaces: 4,667 translations, 19 preserved original metadata entries and two project notices. The release carries 181 new Applied Energistics 2 language entries, for 719 AE2 entries total. The previous accepted scope contains 538 entries. All four final review records are present; the Of record covers its single assigned key. Current native checks cover 37 formatter cases and 17 consumer sequences with visual_qa=0; the public package gate remains separate. Some inputs use explicit fixtures shaped after the source callers; these checks do not establish a full MOD lifecycle or graphical frontend. Existing NeoForge and Balm review evidence is retained. Native ordinary language-asset loading and formatter checks pass; FML early-loader pack application remains unverified, and visual game-screen checks remain 0. The original metadata comprises JEI `_comment`, Jade `__comment` / `jade.metadata`, QuarryPlus `_comment`, Nature's Compass `_comment`, and Enchantment Descriptions `_comment`, `__comment_jei` and 11 `__support_*` entries, plus Measurements `_comment`. Changes comprise Japanese names, descriptions, narration and search terms, with the metadata and project-notice exceptions below. Language modification date: **2026-09-20 (UTC)**.
This is the 0.25.0 release documentation. Game-screen visual QA remains 0, and FML early-loader pack application is unverified.
...
Target: **26.1.10-beta**, 719 language entries across batches 0003–0007 and 0010–0017. The previously accepted AE2 scope contains 538 entries; batches 0004, 0005, 0006 and 0007 add 181 entries. All four final review records for batches 0004, 0005, 0006 and 0007 are present; the Of review is a one-entry partial record because its assigned scope is one key. Current native checks cover 37 formatter cases and 17 consumer sequences with visual_qa=0; the public package gate remains separate. Some inputs use explicit fixtures shaped after the source callers; these checks do not establish a full MOD lifecycle or graphical frontend. The fixed English language entry SHA-256 is `0d9f11ad01d411f780a81fce45e73ef76bb7802abb2a628dc3feb8c37e33a880`; the existing Japanese entry SHA-256 is `405239e3613f55ac56e71ce62835d8fc33e9bb7d5c0c5c646f90840b86322aa8`. Both match the fixed source and installed JAR byte-for-byte. Existing Japanese provenance is retained; the official README identifies Crowdin localization contributors and the upstream contributor record remains the source of attribution.
