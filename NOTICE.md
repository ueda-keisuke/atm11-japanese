...
## 0.17.0 additions / 追加内容

0.17.0では、Mining Gadgetsの86言語項目とMeasurementsの設定説明6項目を追加しました。追加の92項目はすべて訳文です。Measurementsの既存15項目と、他の既存51対象の言語データを保持します。全53対象・3,696項目（翻訳3,676、原文メタデータ19、Apache 4(b)改変通知1）を収録します。ゲーム画面の目視確認は0件です。

Version 0.17.0 adds 92 translated source entries: 86 Mining Gadgets language entries and six Measurements configuration descriptions. Measurements now contains 21 entries, including one preserved source comment. The other 51 previously included namespaces retain their language data. Independent review and native component/loader checks are separate from game-screen visual checks, which remain at 0.

ATM11 Japanese project / **ueda-keisuke** provides this unofficial Japanese language collection for Minecraft 26.1.2 / NeoForge 26.1.2.106. Version 0.17.0 contains 3,696 entries across 53 namespaces: 3,676 translations, 19 preserved original metadata entries and one Apache 4(b) project notice. The original metadata comprises JEI `_comment`, Jade `__comment` / `jade.metadata`, QuarryPlus `_comment`, Nature's Compass `_comment`, and Enchantment Descriptions `_comment`, `__comment_jei` and 11 `__support_*` entries, plus Measurements `_comment`. Changes comprise Japanese names, descriptions, narration and search terms, with the metadata and project-notice exceptions below. Language modification date: **2026-09-20 (UTC)**.
