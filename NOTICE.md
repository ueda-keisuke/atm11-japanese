...
## 0.18.0 additions / 追加内容

0.18.0ではNeoForgeの設定名・説明・警告32項目を追加しました。既存の共通UI46項目を保持し、NeoForgeは計78項目です。他の52対象の言語データと既存のレビューを保持しています。全53対象・3,728項目（翻訳3,708、原文メタデータ19、Apache 4(b)改変通知1）を収録します。ゲーム画面の目視確認は0件です。

Version 0.18.0 adds 32 translated NeoForge configuration language entries, retaining the prior 46 generic configuration UI keys. The other 52 namespaces retain their existing language bytes and review evidence. Independent language review and native loading/consumer checks do not establish game-screen visual verification, which remains at 0.

ATM11 Japanese project / **ueda-keisuke** provides this unofficial Japanese language collection for Minecraft 26.1.2 / NeoForge 26.1.2.106. Version 0.18.0 contains 3,728 entries across 53 namespaces: 3,708 translations, 19 preserved original metadata entries and one Apache 4(b) project notice. The original metadata comprises JEI `_comment`, Jade `__comment` / `jade.metadata`, QuarryPlus `_comment`, Nature's Compass `_comment`, and Enchantment Descriptions `_comment`, `__comment_jei` and 11 `__support_*` entries, plus Measurements `_comment`. Changes comprise Japanese names, descriptions, narration and search terms, with the metadata and project-notice exceptions below. Language modification date: **2026-09-20 (UTC)**.
