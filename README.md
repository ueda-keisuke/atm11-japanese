...
ATM11 で使う一部 MOD の日本語を改善する、非公式リソースパックです。**0.18.0は53のMOD・ライブラリなどを対象に、3,708項目の日本語訳を1つのZIPへ収録しています。** ATM11 全体の翻訳は進行中です。下の一覧にないMODやクエストは収録していません。

## 導入・解除

1. [リリース](https://github.com/ueda-keisuke/atm11-japanese/releases/latest)から **`ATM11-Japanese-0.18.0.zip`** をダウンロードします。非営利条件付きの Jade・Nature's Compass・Better Advancements 素材を含みます。再配布・改変時は下記の個別条件を確認してください。
...
| NeoForge設定 / `neoforge` | 26.1.2.106 | 78（共通UI46＋設定名・説明32） | LGPL-2.1-only |
...
収録項目は全3,728件です。表の件数には表示文、ナレーション、検索補助語のほか、保持したメタデータとApache 4(b)の改変通知も含みます。画面数や目視確認済みの文字列数ではありません。JEI 1・Jade 2・QuarryPlus 1の計4メタデータと、Nature's Compassの原文メタデータ1項目、Enchantment Descriptionsの出典・区切りなど13項目は翻訳せず保持します（計18項目）。加えてMeasurementsの区切りコメント1項目を保持し、メタデータは計19項目です。Simple Backupsの改変通知1項目は翻訳項目ではありません。
...
構文エラーのある **低優先の `transmog:lang/ja_jp.json` だけ**を遮断して同梱の日本語を読み込みます。同じファイルを使う低優先パックの差分も遮断されます。残りの52対象は通常のキー上書きで、読み込みフィルターは使いません。Iron Jetpacksの派生14キーは通常41キーとは別出典・別レビューです。英語原文にない既存日本語キーは、基底リソースから残る場合があります。元 MOD の JAR は変更しません。ExtendedAE のゲーム内ガイド45ページ、別配布の日本語表示補助MOD、画像・モデルは収録しません。FTB UltimineとSimple MagnetsはARR（All Rights Reserved）で翻訳の公開許諾を確認できないため収録しません。Charging Gadgetsの設定4項目は0.11で本体ラベルと説明を追加し、0.12ではNeoForge共通設定46項目にRange表示の日本語を追加しています。NeoForgeの残り188項目や、コードに直接書かれたメニュー名は今回の翻訳対象外です。今回、NeoForgeの設定名・説明32項目を追加し、既存の共通UI46項目と合わせて78項目になりました。設定画面全体の日本語化を意味しません。Simple Backupsの改変通知は表示用翻訳ではなく、Apache 4(b)の通知です。
...
- **NeoForge設定（今回の78項目）**: LGPL 2.1-only。The NeoForged Teamの表示、LGPL全文、固定source/JAR出典、既存JAの出典を保持します。NeoForge本体JARやJavaコード、Minecraft資産、残り188項目は収録しません。
...
Version 0.18 adds 32 NeoForge setting names, descriptions and warnings, for 78 NeoForge language keys in total. Two retained DEPTH_STENCIL attachment entries have no confirmed consumer in the fixed current sources. The separate Japanese display helper 0.3.0-dev covers 46 hardcoded captions; it is not bundled, and game-screen verification remains pending.
