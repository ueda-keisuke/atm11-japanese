...
ATM11 で使う一部 MOD の日本語を改善する、非公式リソースパックです。**0.19.0は53のMOD・ライブラリなどを対象に、3,772項目の日本語訳を収録しています。NeoForgeのFML関連言語64項目を追加し、独立レビューを完了しています。** ATM11 全体の翻訳は進行中です。下の一覧にないMODやクエストは収録していません。

## 導入・解除

1. [リリース](https://github.com/ueda-keisuke/atm11-japanese/releases/latest)から **`ATM11-Japanese-0.19.0.zip`** をダウンロードします。非営利条件付きの Jade・Nature's Compass・Better Advancements 素材を含みます。再配布・改変時は下記の個別条件を確認してください。
...
| NeoForge設定 / `neoforge` | 26.1.2.106 | 142（既存78＋FML64） | LGPL-2.1-only |
...
構文エラーのある **低優先の `transmog:lang/ja_jp.json` だけ**を遮断して同梱の日本語を読み込みます。同じファイルを使う低優先パックの差分も遮断されます。残りの52対象は通常のキー上書きで、読み込みフィルターは使いません。Iron Jetpacksの派生14キーは通常41キーとは別出典・別レビューです。英語原文にない既存日本語キーは、基底リソースから残る場合があります。元 MOD の JAR は変更しません。ExtendedAE のゲーム内ガイド45ページ、別配布の日本語表示補助MOD、画像・モデルは収録しません。FTB UltimineとSimple MagnetsはARR（All Rights Reserved）で翻訳の公開許諾を確認できないため収録しません。Charging Gadgetsの設定4項目は0.11で本体ラベルと説明を追加し、0.12ではNeoForge共通設定46項目にRange表示の日本語を追加しています。NeoForgeの残り124項目や、コードに直接書かれたメニュー名は今回の翻訳対象外です。今回、既存78項目にFMLのMOD一覧24項目、接続状態16項目、読み込み・不一致画面24項目を加え、NeoForgeは142項目です。FML64項目は独立レビュー済みで、通常の言語資産読み込みも検証済みですが、NeoForge全画面の日本語化、FML初期ローダー経路でのパック適用、実ゲーム画面の目視確認を意味しません。固定ソースで呼出し元を確認できないキーや、コメントアウトされた表示経路も含まれます。Mod一覧の状態表示ではコードから渡されるLOADEDがformatterでloadedになる動的値が残ります。Simple Backupsの改変通知は表示用翻訳ではなく、Apache 4(b)の通知です。
...
- **NeoForge設定（既存78項目＋今回のFML64項目）**: LGPL 2.1-only。The NeoForged Teamの表示、LGPL全文、固定source/JAR出典、既存JAの出典を保持します。NeoForge本体JARやJavaコード、Minecraft資産、残り124項目は収録しません。FML64項目は独立レビュー済みで、通常の言語資産読み込みも検証済みですが、FML初期ローダー経路でのパック適用と実ゲーム画面の目視確認は別の制限として残ります。
...
Version 0.19 adds 64 independently reviewed NeoForge FML language entries to the existing 78, for 142 NeoForge language keys in total. Ordinary language-asset loading is verified; FML early-loader resourcepack application and visual game-screen checks remain unverified. Two retained DEPTH_STENCIL attachment entries have no confirmed consumer in the fixed current sources. The separate Japanese display helper 0.3.0-dev covers 46 hardcoded captions; it is not bundled, and game-screen verification remains pending.
